...
async function sendEmail({ to, subject, html }) {
    if (process.env.RESEND_API_KEY) {
        const resp = await fetch('https://api.resend.com/emails', {
            ...
            body: JSON.stringify({ from: 'LOVIGIN <no-reply@lovig.in>', to, subject, html })
        });
        if (!resp.ok) throw new Error(`Resend failed: ${resp.status} ${await resp.text()}`);
        return;
    }
    // (если нужно — добавь SMTP тут)
}
...
// === NEW: GET /api/oidc/password/inspect?token=...
export async function getInspect(pool, req, res, query) {
    try {
...
// POST /api/oidc/password/forgot   { email }
export async function postForgot(pool, req, res) {
    try {
        ...
        if (recent.rowCount > 0) return genericOk(res);
...
// POST /api/oidc/password/reset    { token, newPassword, email? }
export async function postReset(pool, req, res) {
    try {
        ...
        const formEmail = body.email ? String(body.email).trim().toLowerCase() : null;
        ...
        const dbEmail = String(q.rows[0].email || '').toLowerCase();

        if (formEmail && formEmail !== dbEmail) {
            res.writeHead(400, { 'content-type': 'application/json' });
            res.end(JSON.stringify({ error: 'email_mismatch' }));
            return;
        }
        ...
        await pool.query('UPDATE users SET password_hash=$2 WHERE id=$1', [userId, password_hash]);
