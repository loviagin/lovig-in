...
export default function IntClient({ uid }: { uid: string }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        // ВАЖНО: credentials: 'include' — установит interaction-cookie в браузер
        const res = await fetch(`/interaction/${uid}`, {
          credentials: 'include',
          cache: 'no-store',
        });
        if (!res.ok) throw new Error(`GET /interaction failed: ${res.status}`);
  ...
  if (error)   return <main style={{ padding: 24, fontFamily: 'system-ui', color: 'crimson' }}>{error}</main>;
