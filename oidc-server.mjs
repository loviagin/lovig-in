...
import { getAllUsers, getUsersCount, createUser, deleteUser } from './server/routes/users.mjs';
...
`);
        ...
        if (req.method === 'DELETE' && (m = pathname.match(/^\/api\/users\/([^/]+)$/))) {
            return await deleteUser(pool, req, res, m[1]);
        }
